    ...
    >
      <div class="w-full max-w-lg rounded-2xl border border-slate-200/60 bg-white p-6 shadow-xl">
        ...
        <ol class="mt-4 list-decimal space-y-1 pl-5 text-sm text-slate-600">
          <li>
            在「安全 → API Key」创建一个 <code class="rounded bg-slate-50 px-1">ak-</code> 开头的平台
            API Key（权限等同管理员）
          </li>
          <li>在 MCP 客户端按下述配置接入</li>
          <li>
            连接后工具以 <code class="rounded bg-slate-50 px-1">admin_</code> 前缀列出
          </li>
        </ol>
        ...
        <p class="mt-3 text-xs text-amber-600">
          ⚠ 该 Key 权限等同管理员，请妥善保管；本状态仅反映服务端就绪，实际连通请在客户端配置后测试。
        </p>
